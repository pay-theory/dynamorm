...
2. DynamORM is inferring the wrong table name.
...
err := db.TransactWrite(ctx, func(tx core.TransactionBuilder) error {
    tx.Put(user, dynamorm.IfNotExists())
    return nil
})
...
**Cause:**
DynamORM uses the struct field name by default if no tag is present.

**Solution:**
Add `json` tags to your struct. DynamORM respects `json` tags for attribute naming.
...
**Cause:**
You are using `Where()` on a GSI key but forgot to call `Index()`. Without `Index()`, DynamORM assumes you are querying the main table's Primary Key.
...
func init() {
    ...
    db, _ = dynamorm.LambdaInit(&User{}, &Order{})
}
```
...
-   **DynamORM Logs:** Enable detailed logging within DynamORM (if available) to see query plans.
...
**Solution:**
-   Ensure `dynamorm.LambdaInit()` is called once in `init()`.
...
-   **X-Ray Tracing:** Enable X-Ray for all Lambda functions interacting with DynamORM to get detailed trace maps and performance breakdowns.
