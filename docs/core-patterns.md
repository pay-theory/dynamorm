...
func handler(ctx context.Context) error {
    db, _ := dynamorm.New(...) // Re-connects every time! 10x slower.
    return db.Model(&User{}).Create()
}
```
...
type Document struct {
    ...
    Content string
    Version int64  `dynamorm:"version"` // Automatically increments on update
}
```
...
**Solution:** Use `dynamorm.UnmarshalStreamImage` to convert stream images to your models.
...
func handleStream(e events.DynamoDBEvent) {
    for _, record := range e.Records {
        if record.EventName == "INSERT" || record.EventName == "MODIFY" {
            ...
            err := dynamorm.UnmarshalStreamImage(record.Change.NewImage, &user)
            if err != nil {
...
DynamORM is designed to provide significant business value by improving developer efficiency, reducing operational costs, and enhancing application performance and reliability.
...
- **Recommendation:** Always use `dynamorm.NewLambdaOptimized()` or `dynamorm.LambdaInit()` in your `init()` function or global scope.
