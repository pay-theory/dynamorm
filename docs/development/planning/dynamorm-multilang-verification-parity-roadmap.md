# DynamORM: Multi-language Verification Parity Roadmap
...
`docs/development/planning/dynamorm-multilang-feature-parity-matrix.md`.
...
- Each language exposes a **public**, supported mocking surface for:
  - DynamoDB client calls used by DynamORM
...
- TypeScript: ship a `testkit` export (e.g., `@pay-theory/dynamorm-ts/testkit`) with a strict `send()` mock for the AWS
  SDK v3 command surface DynamORM uses.
- Python: ship `dynamorm_py.mocks` with stub clients for DynamoDB + KMS and/or documented `botocore.stub.Stubber`
  helpers.
