# DynamORM Spec (DMS) v0.1 (Draft)

Status: **draft**  
Primary goal: make DynamORM’s behavior portable across languages (starting with TypeScript) without semantic drift.
...
- `docs/development/planning/dynamorm-multilang-roadmap.md`
...
- Replacing DynamoDB’s own documented semantics (this spec only defines DynamORM-specific behavior).
...
Envelope shape (v0.1 aligns with `docs/development/planning/dynamorm-encryption-tag-roadmap.md`):
