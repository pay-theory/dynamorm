...
- Parity tiers: `docs/development/planning/dynamorm-go-ts-parity-matrix.md`
- Multi-lang plan: `docs/development/planning/dynamorm-multilang-roadmap.md`

## Repository strategy (recommended)

Create a dedicated repo: `dynamorm-contract-tests` (or `dynamorm-spec` if you want spec + tests together).

Each implementation repo (`dynamorm` (Go), `dynamorm-ts`) should:
...
- Each language repo implements a small driver wrapper that calls its own DynamORM API.
- The runner:
...
This repo includes an initial seed set so `dynamorm-ts` can start implementing immediately:
...
- cursor encode/decode must match `dynamorm-spec-dms-v0.1` requirements
