# DynamORM: Rubric vNext Notes (v0.4+)
...
- This document now tracks *new* candidates for `v0.4+` and captures lessons learned while hardening the rubric surface.
...
- Introduce an allowlist mechanism that’s explicit and reviewable (e.g., `// dynamorm: allow-panic <reason>`).
...
**Proposed rubric item:** `SEC-5` — “Default `dynamorm.New(...)` path uses memory-safe marshaling; unsafe marshaling is
...
- Grep-based verifier that checks `dynamorm.New` and `NewLambdaOptimized` construct marshalers via a safe factory.
...
**Proposed rubric item:** `SEC-7` (or `QUA-6`) — “HTTP clients constructed by DynamORM have explicit timeouts, and retry
...
When closing a milestone in `docs/development/planning/dynamorm-10of10-roadmap.md`, add a short checklist item:
...
**Proposed addition:**
- Add a unit test that asserts `dynamorm.New(...)` uses safe-by-default marshaling and that any unsafe option is explicit and named accordingly.
