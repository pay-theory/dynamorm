...
try {
  ...
  const page1 = await dynamorm.query('User').partitionKey(pk).limit(2).page();
  ...
  if (!process.env.CI) {
    console.warn(
      `Skipping query integration tests (endpoint unreachable: ${endpoint})`,
    );
    process.exit(0);
  }
  throw err;
