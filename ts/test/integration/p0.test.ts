...
try {
  ...
  const got0 = await dynamorm.get('User', key);
  ...
  await dynamorm.update('User', { ...key, nickname: 'Alice', version: 0 }, [
    'nickname',
  ]);
  const got1 = await dynamorm.get('User', key);
  assert.equal(got1.nickname, 'Alice');
  assert.equal(got1.version, 1);

  await dynamorm.delete('User', key);
  await assert.rejects(
    () => dynamorm.get('User', key),
    (err) => err instanceof DynamormError && err.code === 'ErrItemNotFound',
  );

  // p0.encoding.omitempty
  const id2 = `${id}#omitempty`;
  const key2 = { PK: id2, SK: 'PROFILE' };
  await dynamorm.create('User', { ...key2, nickname: '', tags: [], ttl: 0 });
  const got2 = await dynamorm.get('User', key2);
  ...
  await dynamorm.create('User', { ...key3, nickname: 'v0' });
  const got3a = await dynamorm.get('User', key3);
  ...
  await sleep(25);
  await dynamorm.update('User', { ...key3, nickname: 'v1', version: 0 }, [
    'nickname',
  ]);
  const got3b = await dynamorm.get('User', key3);
  ...
  await dynamorm.create('User', { ...key4, nickname: 'v0' });
  await dynamorm.update('User', { ...key4, nickname: 'v1', version: 0 }, [
    'nickname',
  ]);
  await assert.rejects(
    () =>
      dynamorm.update('User', { ...key4, nickname: 'stale', version: 0 }, [
        'nickname',
      ]),
    (err) => err instanceof DynamormError && err.code === 'ErrConditionFailed',
  );
  const got4 = await dynamorm.get('User', key4);
  ...
  await dynamorm.create('User', { ...key5, ttl: 1_700_000_000 });
  const got5 = await dynamorm.get('User', key5);
