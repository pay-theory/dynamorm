...

{
  ...
  const client = new DynamormClient(mock.client).register(model);
  await client
...

{
  ...
  const client = new DynamormClient(mock.client, {
    encryption: provider,
  }).register(model);
  ...
  await assert.rejects(
    () =>
      client
        ...
        .execute(),
    (e) =>
      e instanceof DynamormError && e.code === 'ErrEncryptedFieldNotQueryable',
  );
}
  ...
  assert.throws(
    () =>
      client
        ...
        .set('name', 'x')
        .conditionVersion(1),
    (e) => e instanceof DynamormError && e.code === 'ErrInvalidModel',
  );
}
...

{
  ...
  await assert.rejects(
    ...
    (e) => e instanceof DynamormError && e.code === 'ErrConditionFailed',
  );
}
