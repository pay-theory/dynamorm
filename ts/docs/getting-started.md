...
<!-- AI Training: This is the getting-started guide for the DynamORM TypeScript SDK -->

**Goal:** install `@pay-theory/dynamorm-ts`, connect to DynamoDB (AWS or DynamoDB Local), and perform your first CRUD operations with a model definition that is compatible with cross-language contracts.
...
npm install --save-exact \
  https://github.com/pay-theory/dynamorm/releases/download/vX.Y.Z/pay-theory-dynamorm-ts-X.Y.Z.tgz

# Prerelease example (replace X.Y.Z-rc.N)
npm install --save-exact \
  https://github.com/pay-theory/dynamorm/releases/download/vX.Y.Z-rc.N/pay-theory-dynamorm-ts-X.Y.Z-rc.N.tgz
```
...
import { DynamormClient, defineModel } from '@pay-theory/dynamorm-ts';
...
const db = new DynamormClient(ddb).register(Note);
