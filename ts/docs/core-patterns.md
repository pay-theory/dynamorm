...
This document provides copy-pasteable patterns for the DynamORM TypeScript SDK.

## Pattern: Define a DMS-friendly model

```ts
import { defineModel } from '@pay-theory/dynamorm-ts';
...
const db = new DynamormClient(ddb).register(User);
...
```ts
import { unmarshalStreamRecord } from '@pay-theory/dynamorm-ts';
