...
function validateModelSchema(schema: ModelSchema): void {
  if (!schema?.name)
    throw new DynamormError('ErrInvalidModel', 'Model name is required');
  if (!schema?.table?.name)
    throw new DynamormError('ErrInvalidModel', 'Model table.name is required');
  ...
  for (const idx of schema.indexes ?? []) {
    if (!idx?.name)
      throw new DynamormError('ErrInvalidModel', 'Index name is required');
