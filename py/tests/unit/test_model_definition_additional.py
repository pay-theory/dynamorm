...
class _Base:
    ...
    sk: str = dynamorm_field(roles=["sk"], default="")
    other: str = dynamorm_field(default="")
...
def test_dynamorm_field_rejects_default_and_default_factory() -> None:
    with pytest.raises(ValueError, match="cannot set both default and default_factory"):
        dynamorm_field(default=1, default_factory=list)  # type: ignore[arg-type]
...
def test_model_definition_rejects_multiple_sk_fields() -> None:
    ...
    class Bad:
        pk: str = dynamorm_field(roles=["pk"])
        sk1: str = dynamorm_field(roles=["sk"])
        sk2: str = dynamorm_field(roles=["sk"])
...
def test_model_definition_rejects_encrypted_index_roles() -> None:
    ...
    class Bad:
        pk: str = dynamorm_field(roles=["pk"])
        secret: str = dynamorm_field(encrypted=True, roles=["index_pk:gsi1"])
