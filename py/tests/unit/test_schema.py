...
from dynamorm_py import ModelDefinition, Projection, ValidationError, dynamorm_field, gsi, lsi
from dynamorm_py.errors import AwsError, NotFoundError
from dynamorm_py.mocks import FakeDynamoDBClient
from dynamorm_py.schema import (
...
def model() -> ModelDefinition[object]:
    ...
    class Record:
        ...
        email_hash: str = dynamorm_field(name="emailHash")
        updated: int = dynamorm_field(name="updated")
...
def test_build_create_table_request_key_type_inference_variants() -> None:
    ...
    @dataclass(frozen=True)
    class BinKey:
        pk: bytes = dynamorm_field(roles=["pk"])
    ...
    @dataclass(frozen=True)
    class OptionalKey:
        pk: str | None = dynamorm_field(roles=["pk"])
    ...
    @dataclass(frozen=True)
    class JsonKey:
        pk: dict[str, int] = dynamorm_field(roles=["pk"], json=True)
    ...
    class UnsupportedKey:
        pk: list[str] = dynamorm_field(roles=["pk"])
