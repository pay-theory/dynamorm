...
class Item:
    ...
    user_id: UserID = dynamorm_field(name="userId", converter=UserIDConverter())
...
@dataclass(frozen=True)
class Keyed:
    pk: UserID = dynamorm_field(name="PK", roles=["pk"], converter=UserIDConverter())
    value: str = dynamorm_field(name="value", default="")
