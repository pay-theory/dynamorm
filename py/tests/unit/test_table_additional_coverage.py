...
from dynamorm_py.model import AttributeDefinition, Projection, gsi
from dynamorm_py.table import _coerce_value, _is_empty, _map_client_error, _map_transaction_error
from dynamorm_py.transaction import (
...
class Item:
    ...
    value: int = dynamorm_field(name="value")
    note: str = dynamorm_field(name="note", default="")
...
def test_query_scan_success_paths_include_index_limit_cursor_and_projection() -> None:
    ...
    class Record:
        pk: str = dynamorm_field(name="PK", roles=["pk"])
        sk: str = dynamorm_field(name="SK", roles=["sk"])
        gsi_pk: str = dynamorm_field(name="gsi_pk")
        value: int = dynamorm_field(name="value")
    ...
    from dynamorm_py.query import encode_cursor
...
def test_internal_helpers_cover_projection_and_serialization_error_paths() -> None:
    ...
    class Minimal:
        ...
        optional: str = dynamorm_field(default="")
...
def test_to_item_and_from_item_error_paths_and_json_unmarshal() -> None:
    @dataclass(frozen=True)
    class MissingPK:
        pk: str = dynamorm_field(roles=["pk"], omitempty=True, default="")
        value: int = dynamorm_field(default=0)
    ...
    class MissingSK:
        pk: str = dynamorm_field(roles=["pk"])
        sk: str = dynamorm_field(roles=["sk"], omitempty=True, default="")
    ...
    class WithIgnoredRequired:
        pk: str = dynamorm_field(roles=["pk"])
        ignored: str = dynamorm_field(ignore=True)
...
def test_from_item_encrypted_envelope_must_be_map() -> None:
    ...
    class Encrypted:
        pk: str = dynamorm_field(roles=["pk"])
        sk: str = dynamorm_field(roles=["sk"])
        secret: str = dynamorm_field(encrypted=True, default="")
