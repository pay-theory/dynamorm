...
from dynamorm_py.dms import _model_definition_to_dms_model
...
class _Demo:
    ...
    secret: str = dynamorm_field(name="secret", encrypted=True, omitempty=True, default="")


def test_parse_dms_document_and_get_model() -> None:
    raw = """
dms_version: "0.1"
namespace: "dynamorm.test"
models:
  - name: "Demo"
    table: { name: "tbl" }
    keys:
...
class _Complex:
    pk: str = dynamorm_field(name="PK", roles=["pk"])
    sk: str = dynamorm_field(name="SK", roles=["sk"])
    email: str = dynamorm_field(name="email")
    ts: int = dynamorm_field(name="ts")
    tags: set[str] = dynamorm_field(name="tags", set_=True, default_factory=set)
    scores: set[int] = dynamorm_field(name="scores", set_=True, default_factory=set)
    blobs: set[bytes] = dynamorm_field(name="blobs", set_=True, default_factory=set)
    blob: bytes = dynamorm_field(name="blob", omitempty=True, default=b"")
    flags: list[str] = dynamorm_field(name="flags", default_factory=list)
    meta: dict[str, int] = dynamorm_field(name="meta", default_factory=dict)
    ok: bool = dynamorm_field(name="ok", default=True)
    note: str | None = dynamorm_field(name="note", omitempty=True, default=None)
    secret: str = dynamorm_field(name="secret", encrypted=True, omitempty=True, default="")
...
@dataclass(frozen=True)
class _BadKeyType:
    pk: list[str] = dynamorm_field(name="PK", roles=["pk"])
...
class _BadSetElement:
    pk: str = dynamorm_field(name="PK", roles=["pk"])
    things: set[object] = dynamorm_field(name="things", set_=True, default_factory=set)
