...
class Item:
    ...
    name: str = dynamorm_field()
    version: int = dynamorm_field(roles=["version"])
    nickname: str = dynamorm_field(default="")
    tags: set[str] = dynamorm_field(set_=True, default_factory=set)
    items: list[object] = dynamorm_field(default_factory=list)
