...
from dynamorm_py import ConditionFailedError, ModelDefinition, NotFoundError, Table, dynamorm_field
...
class Note:
    ...
    note: str = dynamorm_field(omitempty=True, default="")
    payload: dict[str, int] = dynamorm_field(json=True, omitempty=True, default_factory=dict)


def test_table_crud_round_trip_and_conditions() -> None:
    table_name = f"dynamorm_py_table_{uuid.uuid4().hex[:12]}"
    client = _client()
    client.create_table(
