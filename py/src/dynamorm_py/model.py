...
def dynamorm_field(
    *,
    ...
    if default is not MISSING and default_factory is not MISSING:
        raise ValueError("dynamorm_field: cannot set both default and default_factory")

    dynamorm: dict[str, Any] = {
        "omitempty": omitempty,
        "set": set_,
        "json": json,
        "binary": binary,
    ...
    if roles is not None:
        dynamorm["roles"] = list(roles)

    return field(default=default, default_factory=default_factory, metadata={"dynamorm": dynamorm})
    ...
    def from_dataclass(
        ...
        *,
        ...
        for dc_field in fields(model_type):
            opts = cast(dict[str, Any], dc_field.metadata.get("dynamorm", {}))
