...
test('filters + groups + projection escaping + cursor handoff', async (t) => {
  ...
  const dynamorm = new DynamormClient(ddb).register(model);
  const pk = `PK#${Date.now()}`;

  await dynamorm.create('FilterContract', { PK: pk, SK: '0' });
  await dynamorm.create('FilterContract', { PK: pk, SK: '1', tag: 'X', name: 'Alice' });
  await dynamorm.create('FilterContract', { PK: pk, SK: '2', tag: 'Y', name: 'Bob' });

  const q0 = await dynamorm
  ...
  const scanned = await dynamorm.scan('FilterContract').scanAllSegments(2, { concurrency: 1 });
  assert.equal(
