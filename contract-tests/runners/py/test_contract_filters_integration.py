...
from dynamorm_py import FilterCondition, FilterGroup, ModelDefinition, Table, dynamorm_field
...
class FilterItem:
    ...
    name: str | None = dynamorm_field(omitempty=True, default=None)
...
def test_filters_groups_projection_and_cursor_handoff() -> None:
    ...
    table_name = f"dynamorm_contract_filters_{uuid.uuid4().hex[:12]}"
    client = _client()
    client.create_table(
