...
py_version=""
if [[ -f "py/src/dynamorm_py/version.json" ]]; then
  py_version="$(
    python3 - <<PY
import json
from pathlib import Path

data = json.loads(Path("py/src/dynamorm_py/version.json").read_text(encoding="utf-8"))
print(data.get("version", ""))
PY
  )"
fi
...
if [[ -n "${py_version}" ]]; then
  if [[ "${py_version}" != "${expected}" ]]; then
    echo "version-alignment: FAIL (py/src/dynamorm_py/version.json ${py_version} != ${expected} from ${manifest})"
    exit 1
  fi
fi
