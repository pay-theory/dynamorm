...
type cov6UpsertModelOK struct {
	...
	Ignored    string   `dynamorm:"-"`
	Optional   string   `dynamorm:"attr:optional,omitempty"`
	When       struct{} `dynamorm:"attr:when,omitempty"`
	Bad        chan int `dynamorm:"attr:bad,omitempty"`
	unexported string   // should be skipped
}

type cov6UpsertModelBad struct {
	BadHard chan int `dynamorm:"attr:bad_hard"`
	ID      string   `dynamorm:"pk"`
}
