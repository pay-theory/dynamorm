...
func DemonstrateConditionalHelpers() {
	...
	if err != nil {
		log.Fatalf("init dynamorm: %v", err)
	}
...
func DemonstrateTransactionBuilder() {
	...
	err = db.Transact().
		...
		UpdateWithBuilder(quota, func(ub core.UpdateBuilder) error {
			...
			return nil
		}, dynamorm.Condition("Remaining", ">", 0)).
		Put(&BookmarkAudit{
			PK:        bookmark.PK,
	...
	err = db.TransactWrite(ctx, func(tx core.TransactionBuilder) error {
		...
		tx.ConditionCheck(quota, dynamorm.Condition("Remaining", ">=", 0))
		return nil
	})
	if err != nil {
...
func DemonstrateBatchGetBuilder() {
	...
	keys := []any{
		dynamorm.NewKeyPair("ORG#42", "INVOICE#2024-01"),
		dynamorm.NewKeyPair("ORG#42", "INVOICE#2024-02"),
		dynamorm.NewKeyPair("ORG#42", "INVOICE#2024-03"),
		dynamorm.NewKeyPair("ORG#42", "INVOICE#2024-04"),
	}
