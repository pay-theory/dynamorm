...
type Contact struct {
	...
	Email string `dynamorm:"index:gsi-email,pk;required"`

	// GSI for searching by phone
	Phone string `dynamorm:"index:gsi-phone,pk"`

	// Contact details
	FirstName string `dynamorm:"required"`
	LastName  string `dynamorm:"required"`
	Company   string
	JobTitle  string

	// GSI for full name search (LastName#FirstName)
	FullName string `dynamorm:"index:gsi-name,pk"`
...
func (app *ContactsApp) runCLI() {
	...
	fmt.Printf("📇 Welcome to DynamORM Contacts App!\n")
